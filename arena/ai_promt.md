...
**Импорты (пример)**
- Если вы используете сборщик и хотите воспользоваться математикой, можно импортировать:
```js
import { len, sub, rot, norm, dot } from '../engine/math.js';
```
- Но в типичной конфигурации движок вызывает `decide(input)` и боту НЕ НУЖНО ничего импортировать — всё, что нужно, уже в `input`.
